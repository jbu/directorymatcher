import unittest
import directorymatcher1
'''
Run with python -m unittest discover
'''

class TestDMatcher1(unittest.TestCase):

    def test_characterization(self):
        c = directorymatcher1.characterize_file('.', 'lorem')
    ...
    def test_dirmatch(self):
    	...
    	d = directorymatcher1.dir_contents('tdir')
    	self.assertEquals(dat, d)

    def test_dirlist(self):
    	dat = [('tdir/b', 'tdir/a'), ('tdir/c', 'tdir/a')]
    	d = directorymatcher1.match_directories('tdir')
    	self.assertEquals(dat, d)
